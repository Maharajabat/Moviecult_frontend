    ...
    <div className="startup">
        <div className="enterform">
            {login && <form id='loginform'  onSubmit={handlesubmit}>
                <label >Email</label>
            <input type="email" className="email" onChange={(e)=>{setemail(e.target.value); seterror(false);log.disabled=false;
        log.style.opacity="100%";}} style={{"textTransform":"lowercase"}} required/>
            <label >Password</label>
            <input type="password" className="pw" onChange={(e)=>{setpw(e.target.value); seterror(false);log.disabled=false;
